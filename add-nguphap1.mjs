/**
 * add-nguphap1.mjs
 * Seed khóa học "Basic Grammar English" vào Firebase.
 ...
 *
 * Usage: node add-nguphap1.mjs <email> <password> [--dry-run]
...
if (!email || !password) {
    console.error('Usage: node add-nguphap1.mjs <email> <password> [--dry-run]');
    process.exit(1);
}
...
const INPUT_DIR = resolve(__dirname, 'nguphap1');
const COURSE_TITLE_VI = 'Ngữ pháp tiếng Anh cơ bản';
const COURSE_TITLE_EN = 'Basic Grammar English';
const SESSION_TITLE_VI = 'Basic Grammar';
const SESSION_TITLE_EN = 'Basic Grammar';

// UUID pattern: ng{lesson:04d}-0000-4000-8000-000000000000
const makeUUID = (lessonNum) =>
    `ng${String(lessonNum).padStart(4, '0')}-0000-4000-8000-000000000000`;
...
if (!COURSE_ID) {
    if (DRY_RUN) {
        ...
        await setDoc(newCourseRef, {
            ...
            description: { vi: 'Khóa học ngữ pháp tiếng Anh cơ bản với 24 bài học từ cơ bản đến nâng cao' },
            adminId: ADMIN_ID,
            videoCount: 0,
...
for (const mdFile of mdFiles) {
    // Extract lesson number from filename: unit_01_a_an_noun.md → 1
    const numMatch = mdFile.match(/unit_(\d+)_/);
