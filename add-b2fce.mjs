/**
 * add-b2fce.mjs
 * Seed B2-FCE English course lessons vào Firebase.
 ...
 *
 * Usage: node add-b2fce.mjs <email> <password> [--unit=N] [--dry-run]
...
if (!email || !password) {
    console.error('Usage: node add-b2fce.mjs <email> <password> [--unit=N] [--dry-run]');
    process.exit(1);
}
...
// ── Constants ──────────────────────────────────────────────────────────────
const COURSE_ID    = 'FudC7FBvpV3YYZVpVOHO'; // B2-FCE
const ADMIN_ID     = userCred.user.uid;
const BASE         = 'artifacts/video-hub-prod-id/public/data';
const LESSONS_ROOT = 'C:/Users/LongTM4/Documents/english/B2-output';
const NUM_UNITS    = 14;
const UNIT_FOLDER_PREFIX = 'b2-unit'; // folder: b2-unit1, b2-unit2, ...
...
// UUID deterministic: b2{unit:04d}{lesson:02d}-0000-4000-8000-000000000000
const makeUUID = (unit, lesson) =>
    `b2${String(unit).padStart(4,'0')}${String(lesson).padStart(2,'0')}-0000-4000-8000-000000000000`;
