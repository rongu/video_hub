/**
 * add-c1cae.mjs
 * Seed C1-CAE English course lessons vào Firebase.
 ...
 *
 * Lưu ý: Folder tên là c1-unit{N} nhưng file bên trong dùng prefix "unit{N}-..."
 *        (không có "c1-" ở đầu tên file).
 *
 * Usage: node add-c1cae.mjs <email> <password> [--unit=N] [--dry-run]
...
if (!email || !password) {
    console.error('Usage: node add-c1cae.mjs <email> <password> [--unit=N] [--dry-run]');
    process.exit(1);
}
...
// ── Constants ──────────────────────────────────────────────────────────────
const COURSE_ID    = 'lSoVcF3CfvtvGMDnA1hg'; // C1-CAE
const ADMIN_ID     = userCred.user.uid;
const BASE         = 'artifacts/video-hub-prod-id/public/data';
const LESSONS_ROOT = 'C:/Users/LongTM4/Documents/english/C1-output';
const NUM_UNITS    = 14;
const UNIT_FOLDER_PREFIX = 'c1-unit'; // folder: c1-unit1, c1-unit2, ...
// File naming inside folders: unit{N}-{NN}-... (no "c1-" prefix)
...
const UNIT_THEMES = {
     ...
     9: 'Being Somewhere Else',
    10: 'The Living World',
    11: 'Through the Ages',
    12: 'Digital Life',
    13: 'Thrills and Skills',
    14: 'The Hard Sell',
};

// UUID deterministic: c1{unit:04d}{lesson:02d}-0000-4000-8000-000000000000
const makeUUID = (unit, lesson) =>
    `c1${String(unit).padStart(4,'0')}${String(lesson).padStart(2,'0')}-0000-4000-8000-000000000000`;
...
for (const unitNum of units) {
    ...
    for (const mdFile of mdFiles) {
        // C1 files: unit{N}-{NN}-..., e.g. unit1-01-starting-off.md
