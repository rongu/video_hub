...
import { 
    ...
    Volume2, LayoutTemplate, Headphones,
    // ✅ THÊM ICON MỚI
    ...
    return (
        <div className="bg-gray-50 rounded-xl p-4 border border-gray-200 mb-4">
            ...
            {!submitted && selected !== null && (
                <button onClick={() => setSubmitted(true)} className="mt-3 text-xs font-bold uppercase tracking-wider bg-gray-900 text-white px-4 py-2 rounded-lg hover:bg-gray-800 transition">Kiểm tra</button>
            )}
            {submitted && (
...
const QuizView: React.FC<{ data: string; title: string; onComplete?: () => void }> = ({ data, title, onComplete }) => {
    ...
    useEffect(() => {
        try {
            ...
            } else { setError('Định dạng dữ liệu Quiz không hợp lệ.'); }
        } catch (e) { setError('Lỗi khi tải dữ liệu câu hỏi (Invalid JSON).'); }
    }, [data]);
        ...
        return (
            <div className="w-full h-full bg-white p-6 md:p-10 flex flex-col items-center justify-center overflow-y-auto">
                <div className="w-full max-w-lg text-center space-y-6">
                    ...
                    <h2 className="text-3xl font-black text-gray-800 uppercase">Kết quả</h2>
                    <div className="py-6 border-y border-gray-100"><div className="text-6xl font-black text-indigo-600 mb-2">{score}/{questions.length}</div><p className="text-gray-500 font-medium">Đúng {percentage}% câu hỏi</p></div>
                    <button onClick={() => { setSelectedAnswers({}); setShowResult(false); setCurrentQIndex(0); setScore(0); }} className="w-full py-4 bg-indigo-600 text-white rounded-xl font-bold uppercase hover:bg-indigo-700 flex items-center justify-center"><RefreshCcw size={20} className="mr-2"/> Làm lại bài</button>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="w-full h-full bg-white flex flex-col relative">
            <div className="p-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50 flex-shrink-0">
                ...
                <div className="text-xs font-bold bg-white border px-3 py-1 rounded-full text-gray-500">Câu {currentQIndex + 1}/{questions.length}</div>
            </div>
            ...
            <div className="p-4 border-t border-gray-100 bg-white flex justify-between items-center flex-shrink-0">
                <button onClick={() => setCurrentQIndex(prev => Math.max(0, prev - 1))} disabled={currentQIndex === 0} className="px-4 py-2 text-gray-500 font-bold disabled:opacity-30 hover:bg-gray-100 rounded-lg">Trước</button>
                {currentQIndex === questions.length - 1 ? (
                    <button onClick={handleSubmit} disabled={Object.keys(selectedAnswers).length !== questions.length} className="px-6 py-2 bg-indigo-600 text-white rounded-lg font-bold hover:bg-indigo-700 disabled:bg-gray-300 flex items-center"><Check size={18} className="mr-2"/> Nộp bài</button>
                ) : (
                    <button onClick={() => setCurrentQIndex(prev => Math.min(questions.length - 1, prev + 1))} className="px-6 py-2 bg-gray-900 text-white rounded-lg font-bold hover:bg-gray-800 flex items-center">Tiếp theo <ChevronRight size={18} className="ml-1"/></button>
                )}
            </div>
        </div>
    );
...
const CourseDetailPage: React.FC<CourseDetailPageProps> = ({ courseId, onNavigate }) => {
    ...
    const renderContent = () => {
        if (!selectedVideo) return <div className="w-full h-full flex flex-col items-center justify-center text-white/50 p-8 text-center bg-gray-900"><PlayCircle size={48} className="mb-4 opacity-20" /><p className="font-bold uppercase text-xs tracking-widest">Chọn bài học để bắt đầu...</p></div>;
            ...
            return (
                <div className="w-full h-full bg-white p-8 overflow-y-auto custom-scrollbar">
                    <div className="max-w-3xl mx-auto pb-16">
                        <div className="border-b border-gray-100 pb-6 mb-8"><h2 className="text-3xl font-black text-gray-900 mb-2 flex items-center"><LayoutTemplate className="mr-3 text-purple-600" size={32}/> {selectedVideo.title}</h2><p className="text-gray-500 font-medium text-sm">Bài học tương tác</p></div>
                        <div className="space-y-12">
                            {blocks.length === 0 && <p className="text-gray-400 italic text-center">Bài học chưa có nội dung.</p>}
                            {blocks.map((block) => (
            ...
            return (
                <div className="w-full h-full bg-white p-8 overflow-y-auto custom-scrollbar">
                    <div className="max-w-3xl mx-auto pb-10">
                        <div className="border-b border-gray-100 pb-6 mb-8"><h2 className="text-3xl font-black text-gray-900 mb-2 flex items-center"><Headphones className="mr-3 text-purple-600" size={32}/> {selectedVideo.title}</h2><p className="text-gray-500 font-medium text-sm">Bài học Audio</p></div>
        ...
        if (selectedVideo.type === 'text') {
            return (
                <div className="w-full h-full bg-white p-8 overflow-y-auto custom-scrollbar">
                    <div className="max-w-3xl mx-auto pb-10">
                        <div className="border-b border-gray-100 pb-6 mb-8"><h2 className="text-3xl font-black text-gray-900 mb-2 flex items-center"><FileText className="mr-3 text-indigo-600" size={32}/> {selectedVideo.title}</h2><p className="text-gray-500 font-medium text-sm">Bài giảng lý thuyết</p></div>
                        <div className="prose prose-lg prose-indigo max-w-none text-gray-700 leading-relaxed"><ReactMarkdown>{selectedVideo.content || "Đang cập nhật..."}</ReactMarkdown></div>
                        <div className="mt-12 pt-8 border-t border-gray-100 flex justify-center"><button onClick={() => handleMarkComplete(selectedVideo.id, true)} disabled={completedVideoIds.includes(selectedVideo.id)} className={`px-8 py-3 rounded-full font-bold transition flex items-center ${completedVideoIds.includes(selectedVideo.id) ? 'bg-green-100 text-green-700 cursor-default' : 'bg-indigo-600 text-white hover:bg-indigo-700 shadow-lg'}`}>{completedVideoIds.includes(selectedVideo.id) ? <><CheckCircle2 className="mr-2"/> Đã hoàn thành</> : <><Check className="mr-2"/> Đánh dấu đã đọc xong</>}</button></div>
                    </div>
                </div>
            );
        }
        ...
        <div className="min-h-screen bg-white flex flex-col font-sans">
            <header className="bg-white border-b border-gray-100 sticky top-0 z-30">
                <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
                    <button onClick={() => onNavigate(user && isEnrolled ? 'home' : 'landing')} className="flex items-center text-gray-500 font-bold hover:text-indigo-600 transition text-sm uppercase tracking-tighter"><ChevronLeft size={18} className="mr-1"/> Quay lại</button>
                    {isEnrolled && <div className="flex items-center space-x-3 bg-indigo-50 p-2 px-4 rounded-full border border-indigo-100"><div className="bg-gray-200 rounded-full h-1.5 w-24 overflow-hidden"><div className="bg-green-500 h-full transition-all duration-700" style={{ width: `${progressPercentage}%` }} /></div><span className="text-[10px] font-black text-indigo-700 uppercase">{progressPercentage}% hoàn thành</span></div>}
                </div>
            </header>
            ...
            <main className="flex-grow p-6 md:p-10 max-w-7xl mx-auto w-full grid grid-cols-1 lg:grid-cols-3 gap-10">
                <div className="lg:col-span-2 space-y-8">
                        ...
                        {!isEnrolled ? (
                            <div className="w-full h-full relative bg-gray-900">
                                ...
                                <div className="absolute inset-0 flex flex-col items-center justify-center p-8 text-center space-y-6">
                                    ...
                                    <div className="max-w-sm"><h3 className="text-white text-2xl font-black mb-2 uppercase tracking-tight">Bài học đã bị khóa</h3><p className="text-gray-300 text-sm mb-8 font-medium">Khóa học này chưa được ghi danh. Vui lòng liên hệ Admin để kích hoạt tài khoản.</p><button onClick={() => user ? setShowContactModal(true) : onNavigate('login')} className="bg-indigo-600 text-white w-full py-4 rounded-2xl font-black text-xs uppercase tracking-widest shadow-lg hover:bg-indigo-700 transition">{user ? 'Liên hệ Admin để ghi danh' : 'Đăng nhập để đăng ký'}</button></div>
                                </div>
                            </div>
                        ) : renderContent()}
                    </div>
                    <div className="space-y-4">
                        ...
                        <div className="flex items-center space-x-4">
                             {selectedVideo && completedVideoIds.includes(selectedVideo.id) && <span className="bg-green-100 text-green-700 px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest flex items-center"><CheckCircle2 size={12} className="mr-1" /> Đã hoàn thành</span>}
                             <span className="text-gray-400 text-xs font-bold uppercase tracking-widest">Bài {videos.findIndex(v => v.id === selectedVideo?.id) + 1} / {videos.length}</span>
                ...
                <div className="lg:col-span-1">
                    <div className="bg-white rounded-[2.5rem] border border-gray-100 overflow-hidden flex flex-col shadow-sm max-h-[calc(100vh-160px)] sticky top-24">
                        <div className="p-6 border-b border-gray-50 bg-gray-50/50 flex items-center justify-between flex-shrink-0"><h3 className="font-black text-gray-800 text-[10px] uppercase tracking-widest flex items-center"><List size={16} className="mr-2 text-indigo-600"/> Lộ trình học tập</h3><span className="text-[10px] font-black text-gray-400 uppercase">{videos.length} Bài</span></div>
            ...
            <footer className="py-12 border-t border-gray-100 text-center text-gray-400 text-[10px] font-black uppercase tracking-widest"><p>© 2025 VideoHub. Học tập để vươn xa.</p></footer>
        </div>
    );
};
