...
import { ChevronLeft, List, Loader2, CheckCircle2, Circle, Lock, X, Phone, MessageCircle, ArrowRight } from 'lucide-react';
import { 
    type Course, 
    type Video, 
...
const CourseDetailPage: React.FC<CourseDetailPageProps> = ({ courseId, onNavigate }) => {
    ...
    // Chỉ lấy completedVideoIds từ hook
    ...
    useEffect(() => {
        ...
        const unsubVideos = subscribeToVideos(courseId, null, (fetched) => {
            setVideos(fetched);
            if (!selectedVideo && fetched.length > 0) setSelectedVideo(fetched[0]);
            setLoading(false);
        });
                ...
                <div className="lg:col-span-2 space-y-8">
                    ...
                    <div className="aspect-video bg-gray-900 rounded-[2.5rem] overflow-hidden shadow-2xl relative border border-gray-100">
                        {!isEnrolled ? (
                            <div className="w-full h-full relative">
                                ...
                                <div className="absolute inset-0 flex flex-col items-center justify-center p-8 text-center space-y-6">
                                    ...
                                    <div className="max-w-sm">
                                        ...
                                        <p className="text-gray-300 text-sm mb-8 font-medium">Bạn cần đăng ký khóa học này để truy cập vào trình phát video và bắt đầu lộ trình học tập.</p>
                                        <button 
                            ...
                            <iframe src={selectedVideo.videoUrl} title={selectedVideo.title} className="w-full h-full" allowFullScreen />
                        ) : (
                            <div className="w-full h-full flex items-center justify-center text-white/50 font-bold uppercase text-xs">Vui lòng chọn video để xem</div>
                        )}
                    </div>
                    ...
                    <div className="bg-white rounded-[2.5rem] border border-gray-100 overflow-hidden flex flex-col shadow-sm max-h-[calc(100vh-160px)]">
                        <div className="p-6 border-b border-gray-50 bg-gray-50/50 flex items-center justify-between">
                            <h3 className="font-black text-gray-800 text-[10px] uppercase tracking-widest flex items-center">
                                <List size={16} className="mr-2 text-indigo-600"/> Danh sách bài học ({videos.length})
                            </h3>
                        </div>
                        ...
                        <div className="overflow-y-auto flex-grow p-4 space-y-2">
                            {videos.map((video) => (
                                <div 
                                    key={video.id}
                                    onClick={() => isEnrolled && setSelectedVideo(video)}
                                    className={`group p-4 rounded-2xl flex items-center justify-between transition-all duration-300 ${
                                        selectedVideo?.id === video.id 
                                            ? 'bg-indigo-600 shadow-xl shadow-indigo-100' 
                                            : !isEnrolled ? 'opacity-60 cursor-not-allowed grayscale' : 'hover:bg-gray-50 cursor-pointer'
                                    }`}
                                >
                                    <div className="flex items-center space-x-3 overflow-hidden">
                                        {isEnrolled && (
                                            <div className={selectedVideo?.id === video.id ? 'text-white' : 'text-gray-300'}>
                                                {completedVideoIds.includes(video.id) ? <CheckCircle2 size={18}/> : <Circle size={18}/>}
                                            </div>
                                        )}
                                        {!isEnrolled && <Lock size={16} className="text-gray-400" />}
                                        <span className={`text-sm font-bold truncate ${selectedVideo?.id === video.id ? 'text-white' : 'text-gray-700'}`}>
                                            {video.title}
                                        </span>
                                    </div>
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
                    <div className="bg-white w-full max-w-md rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
                        <div className="bg-indigo-600 p-8 text-white relative">
                            ...
                            <h3 className="text-2xl font-black uppercase tracking-tight mb-2">Đăng ký khóa học</h3>
                            <p className="text-indigo-100 text-sm italic">"{course?.title}"</p>
                        </div>
