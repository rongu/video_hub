...
import { Loader2, X, UploadCloud, FileText, FolderPlus, Zap, CheckCircle, ListPlus, Video as VideoIcon } from 'lucide-react';
...
                // 3. Chuẩn bị Tiêu đề cuối cùng (Chỉ dùng tên file)
                const fileContext = (file as any).webkitRelativePath || file.name;
                // Lấy tên file và loại bỏ phần mở rộng
...
                <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">2. Phương thức Tải lên <span className="text-red-500">*</span></label>
...
                <button
                    type="submit"
                    // Điều kiện: Đang upload HOẶC không có files HOẶC KHÔNG CÓ SESSION ID
                    disabled={uploading || files.length === 0 || !selectedSessionId} 
...
                    ) : (
                        <VideoIcon size={20} className="mr-2"/> // Đổi icon thành VideoIcon
                    )}
