...
const buildSessionTree = (sessions: Session[]): SessionNodeStructure[] => {
    ...
    // [QUAN TRỌNG] Sort ngay khi build tree đềEđảm bảo thứ tự hiển thềEđúng với orderIndex
...
const SessionManagerForm: React.FC<SessionManagerFormProps> = ({ 
    ...
    const handleMoveSession = useCallback(async (session: Session, direction: 'up' | 'down') => {
        ...
        try {
            ...
            // 2. Xác định session đích đềEswap
                ...
                <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50 flex-shrink-0">
                    <div>
                        <h3 className="text-xl md:text-2xl font-black text-gray-800 flex items-center uppercase tracking-tight">
                            <Layers className='mr-3 text-[#1A73E8]' size={28}/> Quản lý Session
                        </h3>
                        <p className="text-gray-500 text-xs md:text-sm font-medium mt-1">Khóa học: <span className="text-[#1A73E8] font-bold">{courseTitle}</span></p>
                    </div>
